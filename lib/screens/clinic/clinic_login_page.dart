...
class _ClinicLoginPageState extends State<ClinicLoginPage> {
  ...
  void _login() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (user != null) {
          bool isClinic = await _authService.checkUserRole(user.uid, 'clinics');
          if (isClinic) {
            ...
            if (mounted) _showErrorDialog(AppLocalizations.of(context)!.accessDeniedNotClinic);
          }
        }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(l10n.clinicLogin)),
      body: Center(
